...
namespace mrod_ur_ip_controllers 
{
    ...
    bool VelBasedShapeController::init(hardware_interface::VelocityJointInterface *robot, ros::NodeHandle &n)
    {
		...
		E_ = 210000.0; //youngs_modulus_;
		L0_ = ((double)2)/((double)element_count_); //default_length_;
		...
		// x_master_ setup - initial guess is 0, 0
		x_master_.resize(1);
		x_master_[0].position.x = 0;
		x_master_[0].position.y = 0;
			...
			*/
			// prints
				...
				if (elapsed_time.toSec() >= 1.0){
					...
					// for testing step 2
					/*for (int i = 0; i < index_target_.size(); i++){
					  ROS_INFO_STREAM("At time t=" << (current_time_-starting_time_).toSec() << ", node " << index_target_[i] << ", x=" << initial_shape_[index_target_[i]].position.x << ", y=" << initial_shape_[index_target_[i]].position.y);
					}
					for (int i = 0; i < index_target_.size(); i++){
					  ROS_INFO_STREAM("At time t=" << (current_time_-starting_time_).toSec() << ", desired node " << i << ", x=" << desired_shape_[i].position.x << ", y=" << desired_shape_[i].position.y);
					}
					for (int i = 0; i < index_target_.size(); i++){
					  ROS_INFO_STREAM("At time t=" << (current_time_-starting_time_).toSec() << ", target node " << i << ", x=" << target_shape_[i].position.x << ", y=" << target_shape_[i].position.y);
					}
					ROS_INFO_STREAM("At time t=" << (current_time_-starting_time_).toSec() << ", master pose x=" << x_master_[0].position.x << ", y=" << x_master_[0].position.y);*/
    ...
    void VelBasedShapeController::visionShapeCB(const std_msgs::Float64MultiArrayConstPtr& msg)
    {
		...
		for(int i=0; i<index_target_.size(); i++){
			shape_list_camera_frame[i].position.x = (*msg).data[i*dof_count_];
			shape_list_camera_frame[i].position.y = (*msg).data[i*dof_count_+1];
		}
	...
	void VelBasedShapeController::poseListFrameTransform(const std::vector<geometry_msgs::Pose>& in, std::vector<geometry_msgs::Pose>& out, const std::string from_frame, const std::string to_frame)
	{
		...
		listener_.lookupTransform(to_frame, from_frame, ros::Time::now(), tr);
		for(int i=0; i<in.size(); i++){
			tf::Transform in_tf, out_tf;
			tf::poseMsgToTF(in[i], in_tf);
			out_tf = tr*in_tf;
			tf::poseTFToMsg(out_tf, out[i]);
		}
	}
	...
	void VelBasedShapeController::linearTwistListFrameTransform(const std::vector<geometry_msgs::Twist>& in, std::vector<geometry_msgs::Twist>& out, const std::string from_frame, const std::string to_frame)
	{
		...
		listener_.lookupTransform(to_frame, from_frame, ros::Time::now(), tr); // from already created transform
